...
class FirebaseNotificationService:
    ...
    def __init__(self): 
        # Clé serveur pour l'ancienne API (fallback)
        self.project_id = getattr(settings, 'FIREBASE_PROJECT_ID', 'evangelists-febb9') 
        # Méthode 1: Fichier depuis variable d'environnement
        firebase_json = os.getenv('FIREBASE_SERVICE_ACCOUNT_JSON')
        if firebase_json:
            # Créer un fichier temporaire depuis la variable
            self.service_account_path = self._create_temp_service_account(firebase_json)
        else:
            # Méthode 2: Fichier externe (hors du repo)
